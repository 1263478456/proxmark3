    ...
    if (ak_len == 0 && !collect_nonces && !skip_auth && available_pairs == 0) {
        ...
        do {
            ...
            if (collect_nonces) {
                ...
                max_retries_per_call = ((g_conn.pm3_cmd_data_size - sizeof(uint32_t) * 2) / sizeof(uint64_t)) - 1;
            }
...
static int CmdHF14AMfUCAuthChk(const char *Cmd) {
    ...
    uint32_t max_chunk = (g_conn.pm3_cmd_data_size - MIFAREU3P_CHKKEY_HEADER) / keysize;
...
static int CmdHF14AMfUeSetBlk(const char *Cmd) {
    ...
    if (datalen > (int)(g_conn.pm3_cmd_data_size - 4)) {
        PrintAndLogEx(WARNING, "too many pages for one command: max %d pages (%d bytes). Use " _YELLOW_("`hf mfu eload`") " for larger sets",
                      (int)((g_conn.pm3_cmd_data_size - 4) / MFU_BLOCK_SIZE), (int)(g_conn.pm3_cmd_data_size - 4));
        return PM3_EINVARG;
    }
